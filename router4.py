...
def read_csv(path):
    ...
    for line in table:
        # 5. split it by the delimiter, remove any leading or trailing spaces in each element
        row = line.strip().split(',')

        # 6. append the resulting list to table_list.
        table_list.append(row)

    # 7. Close the file and return table_list.
    table_file.close()
    return table_list
...
def generate_forwarding_table_with_range(table):
    ...
    for row in table:
        ...
        if row[0] != '0.0.0.0':
            ...
            # 7. Build the new row.
            new_row = [ip_range[0], ip_range[1], row[2], row[3]]  # Min IP, Max IP, Gateway, Interface
...
def ip_to_bin(ip):
    ...
    for octet in ip_octets:
        ...
        bin_octet_string = bin_octet[2:]  # Removes "0b"
...
def receive_packet(connection, max_buffer_size):
    ...
    try:
        ...
        # If the received data is an empty byte string, assume the connection was closed.
        if received_packet == b'':  
            print("DEBUG: Connection closed. No data received.")
            return None
    except Exception as e:
    ...
    with open('./output/received_by_router_4.txt', 'a') as f:
...
def write_to_file(path, packet_to_write, send_to_router=None):
    ...
    with open(path, "a") as out_file:
        ...
        # 3. Else if this router is sending, then append the intended recipient, along with the packet, to the output
        # file.
        else:
...
def start_server():
    ...
    port = 8004       # the port router 2 will listen on
    soc = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    ...
    soc.listen(5)   # Allow 5 pending connections
    ...
    while True:
        ...
        try:
            Thread(target=processing_thread, args=(connection, ip, port, forwarding_table_with_range, default_gateway_port,5120)).start()
        except Exception as e:
...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    while True:
        ...
        if str(sending_port) == '8005':
            print("sending packet", new_packet, "to Router 5")
            write_to_file('./output/sent_by_router_4.txt', new_packet, send_to_router="5")
            router_5_socket = create_socket("127.0.0.1", 8005)
            router_5_socket.sendall(new_packet.encode('utf-8'))
            router_5_socket.close()
        elif str(sending_port) == '8006':
            print("sending packet", new_packet, "to Router 6")
            write_to_file('./output/sent_by_router_4.txt', new_packet, send_to_router="6")
            router_6_socket = create_socket("127.0.0.1", 8006)
            router_6_socket.sendall(new_packet.encode('utf-8'))
            router_6_socket.close()
